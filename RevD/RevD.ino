...
float I, V, P, A = 0, B, R,Pset,Rset=10000,Temperatura,Vbat,C;    //Variables varias
...
float Iset,Vcut=0,Iprom=0,mAh,mWh,Vprom=0,Pprom;                                                       //Variables varias
long double tiempo5,Itotal=0,Vtot;                                                                 //Tiempo con signo para la rampa
...
#define Voff 0//0.13                                                                    //Correccion del offset de la tension
#define Icorr 1.074                                                                  //Correccion de la lectura de corriente
#define Ioff 13                                                                      //Correccion del offset de la corriente
unsigned long tiempo,tiempo2,tiempo4,Time,tiempo3,muestras,tiempo6,Treset=0,tiempo7;                                  //Tiempos varios
bool Alerta=0,noDisp=0,RampaNoConf=1,AlertaLatch,set,Erun=1,Reset=0,Wait;                                //Variables varias
...
void setup() {
 ...
 // Setpoint = 20;                                              //Temperatura a la que arranca el cooler (45)
    pinMode(5,INPUT);
...
void Cuentas()
{
  ...
  Vbat = C / 100;
  if(V!=0&&Iset!=0) {
     I =Ioff+Icorr* I * 1000 * 5 / (1024*R1);     //Calculamos la corriente en mA
     if(I<200)I=I-Ioff;
     if(I>200&&I<250)I=I-Ioff/2;
  }
  else I=0;
  V =Voff+(Vcorr*V*Max/1024);                  //Calculamos la tensión en V
  Vbat =Voff+(0.96*Vbat*5/1024);          //Calculamos la tensión en Vbat
...
/*void Cooler()
{
  Input =(5*analogRead(PIN_INPUT))/10.24;              //Se controla al cooler
  myPID.Compute();
...
void Temp()
{
  Temperatura=(5*analogRead(PIN_INPUT))/10.24;          //Se controla la temperatura
  if(Temperatura>AT)
  {
    Alerta=1;
    digitalWrite(7,LOW);
...
void serial()
{
  ...
  Serial.print(Iset);
  Serial.print(" mA   ");
  Serial.print(Vbat);                 //Se imprimen los datos por serial
  Serial.print(" V     ");
  Serial.print(mAh);
  Serial.print(" mAh  ");
  Serial.print(mWh);
...
void VcutShow(){                    //Se agregarn ceros y espacios necesarios para mostrar correctamente la tension seteada      
 ...
 void VbatShow(){                    //Se agregarn ceros y espacios necesarios para mostrar correctamente la tension seteada      
      lcd.setCursor(10,1);          // y se la imprime por pantalla
        lcd.print(" ");
        lcd.print(Vbat);
        lcd.print("V");   
